 ...
 */
@Component
public class NewUpdateSpStatusRemindJob implements Job {

	private static Logger logger = (Logger) LoggerFactory.getLogger(NewUpdateSpStatusRemindJob.class);
...
//				DingdingUtil.setMessage(Constants.TASK_ONE_DD_SP_URL_CODE, "系统时间：" + DateUtils.parseToDateTimeStr(new Date()) + ";单笔芝麻信用评分处理异常，申请单编号：" + applicationNumber + "; 错误信息：-->" + e.getMessage());
	...
	public static Object getObject() {
		NewUpdateSpStatusRemindJob instance = (NewUpdateSpStatusRemindJob) SpringContextUtils.getBean(NewUpdateSpStatusRemindJob.class);
		return instance;
	}
