 ...
 */
@Component
public class RedisFraudUtils {

    static JedisPool jedisPool;

    public static final int THREE_DAY_SECOND = 3 * 24 * 3600;

    @Autowired(required=true)
    @Qualifier(value="jedisPoolTwo")
    public void setJedisPool(JedisPool jedisPool) {
    	RedisFraudUtils.jedisPool = jedisPool;
	}
	 ...
	 * 			
	 */
	public static void hmset(String key, HashMap<String, String> map) {
		Jedis jedis = null;
		try {
			jedis = getResource();
			// 获取redis中的value
			jedis.hmset(key, map);
			/*jedis.expire(key, THREE_DAY_SECOND);*/
		} catch (Throwable e) {
			// 异常情况释放redis对象
			returnBrokenResource(jedis);
			e.printStackTrace();
