 ...
 */
@Service
public class SysParamRedisServiceImpl implements SysParamRedisService {

	private Logger logger = LoggerFactory.getLogger(SysParamRedisServiceImpl.class);
	...
	public SysParam querySysParamByParamValueRedis(String paramValue) {
		...
		// 判断系统参数信息是否已存在
		String value = RedisUtils.hget(RedisCons.RISK_SYS_PARAM_INFO, paramValue);
	...
	private SysParam refreshSysParamByParamValueRedis(String paramValue) {
		SysParam sysParam = sysParamDao.queryByParamValueAndStatue(paramValue);//根据参数编号查询状态为正常的参数
		if (sysParam == null) {
		...
		RedisUtils.hsetEx(RedisCons.RISK_SYS_PARAM_INFO, paramValue, SysParamUtils.ObjectToJson(sysParam), 31536000);
		return sysParam;
	}
