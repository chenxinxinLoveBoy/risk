...
@Service
public class BusinessParamServiceImpl implements BaseService<SysParam> {
	@Autowired
	private BusinessParamDao businessParamDao;
	@Autowired
	private SysParamHisDao sysParamHisDao;
	private static Logger logger = LoggerFactory.getLogger("BusinessParamServiceImpl");
	...
	public SysParam queryByParamValue(String paramValue) {
		return businessParamDao.queryByParamValue(paramValue);
	}

	@Override
	public List<SysParam> findAll(SysParam sysParam) {
		// TODO Auto-generated method stub
		return businessParamDao.getAll(sysParam);
	}
	...
	public SysParam getEntityById(Integer sysParamId) {
		// TODO Auto-generated method stub
		return businessParamDao.selectByPrimaryKey(sysParamId);
	}
	...
	public boolean saveEntity(SysParam sysParam) {
		...
		boolean flag = businessParamDao.insertSelective(sysParam);
		// 更新redis,放入缓存,设置失效时间1年 = 60*60*24*365 = 31536000 s
		RedisUtils.hsetEx(RedisCons.RISK_BUSINESS_PARAM_INFO, sysParam.getParamValue(), SysParamUtils.ObjectToJson(sysParam),31536000);
		sysParamHisDao.saveBusinessEntity(sysParam);// 存入历史表
	...
	public boolean updateEntity(SysParam sysParam) {
		...
		SysParam sysOld = businessParamDao.selectByPrimaryKey(sysParam.getSysParamId());
		
		if(sysOld == null){
			throw new RuntimeException("要修改的记录不存在");
		}
		
		boolean flag = businessParamDao.updateByPrimaryKeySelective(sysParam);
		
		SysParam sysNew = businessParamDao.selectByPrimaryKey(sysParam.getSysParamId());
		sysNew.setSysParamId(sysNew.getBusiness_param_id());
		sysParamHisDao.saveBusinessEntity(sysNew);// 存入历史表
		
		String paramValue = sysOld.getParamValue();
		//判断redis中key是否存在，存在便删除该key
		if(RedisUtils.hexists(RedisCons.RISK_BUSINESS_PARAM_INFO, paramValue)){
			RedisUtils.hdelEx(RedisCons.RISK_BUSINESS_PARAM_INFO, paramValue);
		}
		// 更新redis,放入缓存,设置失效时间1年 = 60*60*24*365 = 31536000 s
		RedisUtils.hsetEx(RedisCons.RISK_BUSINESS_PARAM_INFO, sysNew.getParamValue(), SysParamUtils.ObjectToJson(sysNew),31536000);
	...
	public int listAllCount(SysParam sysParam) {
		// TODO Auto-generated method stub
		return businessParamDao.listAllCount(sysParam);
	}
