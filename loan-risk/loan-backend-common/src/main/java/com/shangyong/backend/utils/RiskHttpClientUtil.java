    ...
    public static String doGet(String url, Map<String, String> param, RequestConfig config) {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                ...
                scAlarmImpl.contains(AlarmCodeEnum.SYSTEM_CONFIG, "HTTP【doGet】-->POST-->URL:" + url + "响应状态码：" + response.getStatusLine().getStatusCode(), AlarmThirdPartyCreditInvestigationEnum.SYSTEM_CONFIG);
                //DingdingUtil.setMessage(Constants.WEB_DD_SYS_URL_CODE, "系统时间：" + DateUtils.parseToDateTimeStr(new Date()) + ";HTTP【doGet】-->POST-->URL:" + url + "响应状态码：" + response.getStatusLine().getStatusCode());
...
//                throw new RuntimeException("HttpClientUtil>doGet>response="+response);
//            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static String doPost(String url, Map<String, String> param, RequestConfig config, String contentType) {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                logger.error(String.format("POST接口异常，请求返回状态为：【%s】", response.getStatusLine().getStatusCode()));
                scAlarmImpl.contains(AlarmCodeEnum.APP_DOCKING, "HTTP【doPost】-->POST-->URL:" + url + "响应状态码：" + response.getStatusLine().getStatusCode(), AlarmThirdPartyCreditInvestigationEnum.APP_DOCKING);
                //DingdingUtil.setMessage(Constants.DUBBO_DD_APP_URL_CODE, "系统时间：" + DateUtils.parseToDateTimeStr(new Date()) + ";HTTP【doPost】-->POST-->URL:" + url + "响应状态码：" + response.getStatusLine().getStatusCode());
//    			throw new RuntimeException(String.format("POST接口异常，请求返回状态为：【%s】", response.getStatusLine().getStatusCode()));
            } else {
                resultString = EntityUtils.toString(response.getEntity(), "utf-8");
            }
    ...
    public static String doPostJson(String url, String json) {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                logger.error(String.format("POST接口异常，请求返回状态为：【%s】", response.getStatusLine().getStatusCode()));
                scAlarmImpl.contains(AlarmCodeEnum.APP_DOCKING, "HTTP【doPostJson】-->POST-->URL:" + url + "响应状态码：" + response.getStatusLine().getStatusCode(), AlarmThirdPartyCreditInvestigationEnum.APP_DOCKING);
                //DingdingUtil.setMessage(Constants.DUBBO_DD_APP_URL_CODE, "系统时间：" + DateUtils.parseToDateTimeStr(new Date()) + ";HTTP【doPostJson】-->POST-->URL:" + url + "响应状态码：" + response.getStatusLine().getStatusCode());
//    			throw new RuntimeException(String.format("POST接口异常，请求返回状态为：【%s】", response.getStatusLine().getStatusCode()));
            } else {
                resultString = EntityUtils.toString(response.getEntity(), "utf-8");
            }
     ...
     * @throws Exception
     */
    public static String doPostBQS(String url, Map<String, String> param, RequestConfig config, String contentType) throws Exception {
        // 创建Httpclient对象  
        CloseableHttpClient httpClient = HttpClients.createDefault();
        CloseableHttpResponse response = null;
        String resultString = "";
...
//            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
//            	logger.info(String.format("请求白骑士成功，状态=【%s】", response.getStatusLine().getStatusCode()));
//            	resultString = EntityUtils.toString(response.getEntity(), "utf-8");
//			}else {
//				logger.error(String.format("POST请求白骑士接口异常，请求返回状态为：【%s】", response.getStatusLine().getStatusCode()));
//				throw new RuntimeException(String.format("POST请求白骑士接口异常，请求返回状态为：【%s】", response.getStatusLine().getStatusCode()));
//			}
        } catch (Exception e) {
            logger.error(e.getMessage());
            throw new Exception(e.getMessage());
...
//    			throw new RuntimeException(String.format("POST接口异常，请求返回状态为：【%s】", response.getStatusLine().getStatusCode()));
            ...
            scAlarmImpl.contains(AlarmCodeEnum.APP_DOCKING, "HTTP【doPostJson】-->POST-->URL:" + url, AlarmThirdPartyCreditInvestigationEnum.APP_DOCKING);
            //DingdingUtil.setMessage(Constants.DUBBO_DD_APP_URL_CODE, "系统时间：" + DateUtils.parseToDateTimeStr(new Date()) + ";HTTP【doPostJson】-->POST-->URL:" + url);
        }
        return result;
    }
