...
@Controller
@RequestMapping("/backend/sysParamHis/")
public class SysParamHisController extends BaseController {
	private static Logger logger = Logger.getLogger(SysParamHisController.class);
	@Autowired
	private SysParamHisServiceImpl sysParamHisService;
	...
	public void getAllSysParamHis(HttpServletRequest request, HttpServletResponse response, SysParam sysParam) {
		try {
			int count = sysParamHisService.listAllCount(sysParam);// 统计总条数
			List<SysParam> list = sysParamHisService.findAll(sysParam);
	...
	public void getEntityById(HttpServletRequest request, HttpServletResponse response, SysParam sysParam) {
		try {
			SysParam sysParamHisBo = sysParamHisService.getEntityById(sysParam.getParamHisId());
