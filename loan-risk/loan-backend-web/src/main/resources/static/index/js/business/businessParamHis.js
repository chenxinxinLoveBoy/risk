			...
			layui.use(['paging', 'layer', 'form', 'element','laydate'], function() {
				var $ = layui.jquery,
					...
					element = layui.element(),
				 	url = getCtxPath() + "/backend/BusinessParamHis/getAllSysParamHis.do?v=" + new Date().getTime();// 请求服务器的url
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
								layer.open({
									  ...
									  maxmin: true, // 允许屏幕最小化
									  anim: 2,
									  content: ['businessParamHis_add.html?id='+$(this).data('id')], //iframe的url，no代表不显示滚动条
								});
							});
						});
