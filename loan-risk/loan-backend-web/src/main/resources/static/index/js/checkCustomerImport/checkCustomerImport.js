								 ...
								 $('#content').children('tr').each(function() {
									 ...
									 var $that = $(this); 
										$that.children('td').children('a[data-opt=file]').on('click', function() {
											location.href=getCtxPath() + "/backend/blacklist/download.do?v=" + new Date().getTime()+"&path="+$(this).data('id')+"&fileName=征信源";	
										})
								});
							}						
						});
			...
			function uploadFiel(){
				...
				if($("input[type='checkbox']").is(':checked') == false){
					...
					layer.msg("请选择征信机构！" , {icon: 2});
					return;
				}
				
					$("#uploadStatus").html("<font>文件上传中...</font>");
					/********************************************/
					var FileController=hostIp+ "/checkCustomer/upload.do?v=" + new Date().getTime();// 接收上传文件的后台地址   (请求地址)
				 ...
				 if (xmlhttp.readyState==4){
					 ...
					 if (xmlhttp.status==200){ //200 = OK
						 ...
						 if(jsons.code == "200"){
							 ...
							 console.log(jsons);
							 $("#uploadStatus").html("<font style='color:green'>文件上传成功!批次号为"+jsons.list.data.codeId+"</font>");
						 }else{
							 //  $("#button_save").removeAttr("disabled");// 提交状态，按钮可用
							 $("#uploadStatus").html("<font style='color:red'>文件上传失败!</font>");
						 }
					 }else{
			...
			function dowloadCSV(){
				location.href=getCtxPath() + "/index/views/checkCustomerImport/model.CSV";
			}
