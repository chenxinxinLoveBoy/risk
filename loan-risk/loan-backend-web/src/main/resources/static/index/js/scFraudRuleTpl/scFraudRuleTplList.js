			...
			var look = getQueryString('look'); 
				if(look != '' && look != undefined ){
					$("#getSelected").hide();
 					//当前模版信息下访问路径
					url = getCtxPath() + "/backend/scFraudRule/getTemplate.do?v=" + new Date().getTime();
				}else{
					//查询不匹配模版信息路径
					$("#getSelected").show();
					url = getCtxPath() + "/backend/scFraudRule/getAllTemplate.do?v=" + new Date().getTime();
				}
			
				// 当前菜单名称
				var menuNames = "欺诈规则模版配置";
			function getList(_pageIndex){ 
				...
				var fraudRuleTplId = getQueryString('fraudRuleTplId');//模板id
				...
				layui.use(['paging', 'layer', 'form', 'element'], function() {
	                ...
	                paging.init({
						...
						params: {  
							...
							menuNames : menuNames,
							functionNames: (fraudRuleTplId != ''  ? "修改欺诈规则模版配置" : "新增欺诈规则模版配置") // 按钮功能名称 不为空修改 为空 新增
						},
						type: 'post',
						tempElem: '#tpl', //模块容器
						pageConfig: { //分页参数配置
						...
						complate: function() { //完成的回调 
							...
							$('#getSelected').on('click', function() {
								...
								$.ajax({
									url: getCtxPath() + "/backend/scFraudRule/saveScFraudRuleBatch.do?v=" + new Date().getTime(),
									method: "post",
									dataType: "json",
									data:{
										fraudRuleTplId : fraudRuleTplId, 
										ids : ids,
										menuNames : menuNames,
										functionNames: (ids != ''  ? "批量关联欺诈规则模版配置" : "") // 按钮功能名称 不为空修改 为空 新增
									},
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
									...
									layer.open({
										  ...
										  maxmin: true, // 允许屏幕最小化
										  anim: 2,
										  content: ["../scFraudRule/scFraudRule_add.html?id="+$(this).data('id')], //iframe的url，no代表不显示滚动条
										});
								});
...
//								$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
//									var id= $(this).data('id'); 
//									var href = getCtxPath() + "/index/views/scFraudRule/scFraudRule_add.html?id="+$(this).data('id');
//									var data = {
//										"title": "编辑"+id,
//										"icon": "&#xe60c;",	
//										"href": href
