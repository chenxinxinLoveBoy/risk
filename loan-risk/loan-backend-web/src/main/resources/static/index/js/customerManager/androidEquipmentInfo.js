...
*/
			...
			$(function(){
				...
				//var customerId = 'b88a428b85754ecc81e5563115e7f637';//获取页面传过来的平台用户编号 
				 console.log(customerId);
				layui.use(['paging', 'layer', 'form', 'element'], function() {
	                ...
	                paging.init({
	                    openWait: true,
						url : getCtxPath() + "/backend/androidEquipmentInfo/findListById.do?v=" + new Date().getTime(),// 请求服务器的url
						elem: '#content', //内容容器
						params: { //发送到服务端的参数
							customerId : customerId
						},
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=find]').on('click', function() {									 
									layer.open({
										  ...
										  maxmin: true, // 允许屏幕最小化 
										  anim: 2,
										  content: ['androidEquipmentInfo_detail.html?find=1&id='+$(this).data('id')],
										});
								});
