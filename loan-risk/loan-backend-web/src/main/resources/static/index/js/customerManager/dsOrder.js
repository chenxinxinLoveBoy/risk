...
$(function() {
	...
	function getList(pageIndex, pageSize, platformCustomerId, applicationId) {
		$.ajax({
			url : getCtxPath() + "/backend/MhDs/getOrderInfo.do",
			method : "post",
			dataType : "json",
			data : {
			...
			success : function(data) {
				...
				layui.each(data.list, function(index, item){
				      arr.push('<tr>');
				      arr.push('<td>'+ item.orderId + '</td>');
				      arr.push('<td>'+ item.orderAmount + '</td>');
				      arr.push('<td>'+ item.orderType + '</td>');
				      arr.push('<td>'+ item.orderStatus + '</td>');
				      arr.push('<td>'+ item.orderTime + '</td>');
				      arr.push('</tr>');
				    });
