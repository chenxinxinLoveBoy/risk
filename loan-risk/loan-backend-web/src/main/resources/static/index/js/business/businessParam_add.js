			 ...
			 var layer = layui.layer
			  ...
			  form.on('submit(save)', function(data){
				...
				if(id === undefined || id === ""){
		  		...
		  		$.ajax({
					url :getCtxPath() + "/backend/businessParam/getEntityById.do",
					method: "post",
					dataType:"json",
					data: {
						sysParamId: id
					...
					success: function(data){
						...
						$("#sysParamId").val(data.data.sysParamObject.business_param_id); 
...
function save(form){
	...
	var  frameindex= parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
	$.ajax({
		url : getCtxPath() + "/backend/businessParam/saveBusinessParam.do",
		method : 'post',
		dataType : 'json',
		data: {
			...
			functionNames: (sysParamId != ''  ? "修改业务参数" : "新增业务参数") // 按钮功能名称 不为空修改 为空 新增
		},
