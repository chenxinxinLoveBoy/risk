			...
			var scoreTplName = "", scoreTplCode = "",state = "";
			// 条件搜索
			 function sel() {
			 ...
			 function add() {  
				 layer.open({
					  ...
					  fixed: false, //不固定
					  maxmin: true, // 允许屏幕最小化
					  content: ['scScoreTpl_add.html'], //iframe的url，no代表不显示滚动条
					});
 				}; 
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td').children('a[data-opt=edit]').on('click', function() {
									...
									layer.open({
										  ...
										  maxmin: true, // 允许屏幕最小化 
										  anim: 2,
										  content: ['scScoreTpl_add.html?scoreTplId='+id], //iframe的url，no代表不显示滚动条
										});
								});
							});
...
//									var tplPercent= $(this).data('percent'); 
									...
									// ajax 请求后台
										$.ajax({
											url : getCtxPath() + "/backend/scScoreTpl/saveScScoreTpl.do",
											method : 'post',
											dataType : 'json',
											data: {
												scoreTplId: $(this).data('id'), // 传到后台的参数，子窗口的form表单
												state:state,
												version : version,
//												level : level,
//												tplPercent:tplPercent,
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=find]').on('click', function() {
									...
									var href = getCtxPath() + "/index/views/scScoreTpl/scScoreTplList.html?scoreTplId="+$(this).data('id');
									var data = {
										"title": name+"下可添加评分小类",
										"icon": "&#xe60c;",	
										"href": href
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td').children('a[data-opt=look]').on('click', function() {
									var id= $(this).data('id');
									var name =  $(this).data('name'); 
									var href = getCtxPath() + "/index/views/scScoreTpl/scScoreTplList.html?look=1&scoreTplId="+$(this).data('id');
									var data = {
										"title": name+"下已关联评分小类",
										"icon": "&#xe60c;",	
										"href": href
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=tree]').on('click', function() {
									var id= $(this).data('id');
									var name =  $(this).data('name'); 
									var href = getCtxPath() + "/index/views/scScoreTree/scScoreTree.html?scoreTplId="+$(this).data('id');
									var data = {
										"title": name+"下树形图",
										"icon": "&#xe60c;",	
										"href": href
