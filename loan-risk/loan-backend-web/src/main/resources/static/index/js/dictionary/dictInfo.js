			...
			/**新增  start*/
			 function add() { 
				layer.open({
					  ...
					  anim: 2,
					  content: ['editdictInfo.html'], //iframe的url，no代表不显示滚动条
					});
			};
			...
			function getList(){ 
				...
				layui.use(['layer','upload','paging', 'form'], function() {
	                ...
	                paging.init({
	                    openWait: true,
						url: hostIp+ "/scDicBiglist/list.do?v=" + new Date().getTime(),// 请求服务器的url
						elem: '#content', //内容容器
						params: { //发送到服务端的参数 
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=find]').on('click', function() {									 
									layer.open({
										  ...
										  maxmin: true, // 允许屏幕最小化 
										  anim: 2,
										  content: ['dictInfoList.html?find=1&id='+$(this).data('id')],
										});
								});
