...
layui.use(['layer', 'form', 'laydate', 'paging', 'element'], function () {
    ...
    initPageing();
    initCountHbaseAndartificialNumber();
    initCount();
    ...
    function initPageing() {
        ...
        paging.init({
            openWait: true,
            url: getCtxPath() + "/backend/application/countStepNumber.htm?v=" + new Date().getTime(),
            elem: '#content', //内容容器
            params: params,
            type: 'get',
            tempElem: '#tpl', //模块容器
    ...
    function initCountHbaseAndartificialNumber() {

        var params = returnSelectCondition();

        $.ajax({
            url: getCtxPath() + "/backend/application/countHbaseAndartificialNumber.htm?v=" + new Date().getTime(),
            method: "get",
            dataType: "json",
            data: params,
            success: function (data) {
                if (data.code == 200) {
                    $("#countHbasePendingeview").text(data.data.data.hbaseNumber);
                    $("#countToBeAuditedManually").text(data.data.data.artificialNumber)
                    $("#countNumber").text(data.data.data.countNumber)
                    return;
                }
