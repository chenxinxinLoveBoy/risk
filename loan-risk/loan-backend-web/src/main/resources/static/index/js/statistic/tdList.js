			...
			$('#histogram').on('click', function() {
				...
				var endTime1 = $("#endTime").val().trim(); 
					layer.open({
					  ...
					  maxmin: true, // 允许屏幕最小化
//					  anim: 2,
					  content: ['tdHistogram.html?beginTime='+beginTime1+'&endTime='+endTime1], //iframe的url，no代表不显示滚动条
					});
			});
			function getList(){
				layui.use(['paging', 'layer', 'form', 'element'], function() {
	                ...
	                paging.init({
	                    openWait: true,
						url: getCtxPath() + "/backend/td/queryTdResult.do?v=" + new Date().getTime(),
						elem: '#content', //内容容器
						params: { //发送到服务端的参数
							t: Math.random(),
							startTime : startTime,
