			...
			var url = getCtxPath() + "/backend/scFraudRuleHis/getAllScFraudRuleHis.do?v=" + new Date().getTime();// 请求服务器的url
			// 初始化
			$(function(){
			...
			$('#search').on('click', function() {
				...
				getList(fraudRuleCode,ruleName,creditType);
			});
			
			function getList(fraudRuleCode, ruleName,creditType){ 
				var creditType = $("#creditType").val().trim();
				if(creditType=='00'){
					creditType='';
				}
...
//							state: types == 1 ? "" :state,
	...
	//									  time: 2000, //2秒后自动关闭
										  anim: 2,
										  content: ['scFraudRuleHis_add.html?id='+$(this).data('id')], //iframe的url，no代表不显示滚动条
										});
								});
