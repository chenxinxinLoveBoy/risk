			...
			var urls = getCtxPath() + "/backend/zhiMaRiskCode/list.do?v=" + new Date().getTime();
			// 当前菜单名称
			...
			$('#search').on('click', function() {
				info = $("#info").val().trim();
				code = $("#code").val().trim();
				suggestType = $("#suggestType").val()  == "-1" ? "" : $("#suggestType").val();
			...
			$('#add').on('click', function() {
				layer.open({
					  ...
					  maxmin: true, // 允许屏幕最小化
					  anim: 2,
					  content: ['zhima_risk_code_edit.html'], //iframe的url，no代表不显示滚动条
					});
			});
			/**新增     end*/
	...
	//									  time: 2000, //2秒后自动关闭
										  anim: 2,
										  content: ['zhima_risk_code_edit.html?riskCodeId='+$(this).data('id'), 'no'], //iframe的url，no代表不显示滚动条
										});
								});
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=del]').on('click', function() {
									...
									layer.confirm('您确定要删除吗？', {
										...
										// 充值密码请求
										$.ajax({
											url: getCtxPath() + "/backend/zhiMaRiskCode/delete.do",
											method: "post",
											dataType: "json",
											data:{
												riskCodeId: idd,
...
//									    layer.close(index);
	...
	//									  time: 2000, //2秒后自动关闭
										  anim: 2,
										  content: ['zhima_risk_code_edit.html?find=1&riskCodeId='+$(this).data('id'), 'no'], //iframe的url，no代表不显示滚动条
										});
								});
