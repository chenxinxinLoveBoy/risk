			...
			var decisionTreeName = "", decisionTreeCode = "",state = "",implementType="";
			// 条件搜索
			 function sel() {
				 decisionTreeName = $("#decisionTreeName").val().trim();
				 decisionTreeCode = $("#decisionTreeCode").val().trim();
				 state = $("#state").val().trim();
				 implementType=$("#implementType").val().trim();
				getList(1);
			};
			 ...
			 function add() {  
				 layer.open({
					  ...
					  fixed: false, //不固定
					  maxmin: true, // 允许屏幕最小化
					  content: ['scDecisionTree_add.html'], //iframe的url，no代表不显示滚动条
					});
 				}; 
						...
						complate: function() { //完成的回调
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td').children('a[data-opt=look]').on('click', function() {
									...
									layer.open({
										  ...
										  maxmin: true, // 允许屏幕最小化 
										  anim: 2,
										  content: ['scDecisionTree_look.html?decisionTreeId='+id], //iframe的url，no代表不显示滚动条
										});
								});
							});
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td').children('a[data-opt=edit]').on('click', function() {
									...
									layer.open({
										  ...
										  maxmin: true, // 允许屏幕最小化 
										  anim: 2,
										  content: ['scDecisionTree_add.html?decisionTreeId='+id], //iframe的url，no代表不显示滚动条
										});
								});
							});
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td').children('a[data-opt=update]').on('click', function() {
									...
									// ajax 请求后台
										$.ajax({
											url : getCtxPath() + "/backend/scDecisionTree/saveScDecisionTree.do",
											method : 'post',
											dataType : 'json',
											data: {
												decisionTreeId: $(this).data('id'), // 传到后台的参数，子窗口的form表单
												state:state,
												version : version,
												level : level,
