			...
			var url = getCtxPath() + "/backend/scScoreBigHis/getAllScScoreBigHis.do?v=" + new Date().getTime();// 请求服务器的url
			// 初始化
			$(function(){
			...
			$('#search').on('click', function() {
				...
				getList(scoreName,scoreBigCode,creditType);
			});
			 
			function getList(scoreName, scoreBigCode,creditType){ 
				var creditType = $("#creditType").val().trim();
				if(creditType=='00'){
					creditType='';
				}
	...
	//									  time: 2000, //2秒后自动关闭
										  anim: 2,
										  content: ['scScoreBigHis_add.html?id='+$(this).data('id')], //iframe的url，no代表不显示滚动条
										});
								});
