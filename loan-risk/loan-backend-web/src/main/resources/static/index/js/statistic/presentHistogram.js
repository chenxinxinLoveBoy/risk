...
$(function() { 
	...
	var endTime = decodeURI(getQueryString("endTime")); 
 	...
 	$.ajax({
		url : getCtxPath()+ "/backend/present/application/getPresentHistogram.do",
		method : "post",
		dataType : "json",
		data : {
			startTime : startTime,
		...
		success : function(data) {
			...
			option = {
			    ...
			    yAxis: {
			        type: 'category',
			        data: data.data.presentRiskCheckCountObject.itemName
			    },
			    series: [
			        {
			            name: '出现次数',
			            type: 'bar',
			            data: data.data.presentRiskCheckCountObject.count
			        }
			    ]
			};
