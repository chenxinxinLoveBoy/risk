...
var urls = getCtxPath() + "/backend/mq/log/page?v=" + new Date().getTime();
...
layui.use(['paging', 'layer', 'form', 'element', 'laydate'], function () {
    ...
    $("#resend").click(function () {
        ...
        layer.confirm("您确定要重发吗？", {
            ...
            var hsIds = ""
            $(trs).each(function () {
                var hsId = $(this).find("input[name='hsid']").val();
                hsIds += hsId + ',';
            });
            hsIds = hsIds.substr(0, hsIds.length - 1);
            $.ajax({
                url: getCtxPath() + "/backend/mq/log/resend/",
                method: 'post',
                dataType: "json",
                data: {
                    hsIds: hsIds
    ...
    paging.init({
        ...
        params: {
            ...
            isConsumption: $("#isConsumption").val()
        },
        type: 'post',
        tempElem: '#tpl', //模块容器
        pageConfig: { //分页参数配置
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 200) {
                    ...
                    $("#isConsumption").append("<option value=\"\">全部</option>");
                    ...
                    $.each(data.isConsumption, function (i, n) {
                        $("#isConsumption").append("<option value=\"" + n.code + "\">" + n.name + "</option>");
                    })
                    form.render('select');
                }
            },
