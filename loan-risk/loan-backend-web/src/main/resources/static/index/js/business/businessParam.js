			...
			layui.use(['paging', 'form', 'element'], function() {
				var $ = layui.jquery,
					...
					element = layui.element(),
				 	url = getCtxPath() + "/backend/businessParam/getAllSysParam.do?v=" + new Date().getTime();// 请求服务器的url
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
								layer.open({
									  ...
									  maxmin: true, // 允许屏幕最小化
									  anim: 2,
									  content: ['businessParam_add.html?id='+$(this).data('id')], //iframe的url，no代表不显示滚动条
								});
							});
						});
		...
		layui.use(['layer', 'element'], function() {
			...
			$('#add').on('click', function() {
				layer.open({
					  ...
					  fixed: false, //不固定
					  maxmin: true, // 允许屏幕最小化
					  content: ['businessParam_add.html'], //iframe的url，no代表不显示滚动条
					});
			});
			/**新增     end*/
		});
